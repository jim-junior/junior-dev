...
const {deepMap} = require('../../services/utils/code.utils');
const {filterRootEntries, createFiles, shouldIncludeMetadata} = require('./cms-common-utils');

function buildCMS(ssgType, readwriteToken, options) {
    logger.debug(`Serverless: fetching entries from DatoCMS, readwriteToken: '${readwriteToken.slice(0, 2)}..${readwriteToken.slice(-2)}'`);
    const client = new SiteClient(readwriteToken);
    return fetchData(client).then(cmsData => {
        logger.debug('Serverless: got entries from DatoCMS');
        return filterAndTransformProperties(cmsData, options);
    }).then((entries) => {
        logger.debug('Serverless: generating response data');
        return createFiles(entries, ssgType, options);
    });
}
...
function filterAndTransformProperties(cmsData, options) {
    ...
    const entries = options.allObjects ? cmsData.entries : filterRootEntries(cmsData.entries, _.property('stackbitModelType'));
...
function transformEntries(entries, entriesById, itemTypesById, uploads, site, options) {
    const includeMetadata = shouldIncludeMetadata(options);
    return _.map(entries, entry => {
        return deepMap(entry, ({value: obj, keyPath: fieldPath, objectStack}) => {
            ...
            let result = _.reduce(itemType.fields, (result, field) => {
