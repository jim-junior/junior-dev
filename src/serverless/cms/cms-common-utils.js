...
function createPageFile(page, ssgType, options) {
    ...
    //       instead of returning stackbit_ref_id for page models, return the filePath and make factory to use the
    //       default file-based approach to reference pages. Also need to pass stackbit.yaml version via pull request
    let data = _.omit(page, ['stackbit_model_type', /*'stackbit_url_path',*/ 'stackbit_dir', 'stackbit_file_ext']);
    if (!objectFormat) {
