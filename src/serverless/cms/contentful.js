...
function buildCMS(spaceId, ssgType, accessToken, options) {
    logger.debug(`Serverless: fetching entries from Contentful, spaceId: '${spaceId}', accessToken: '${accessToken.slice(0, 2)}..${accessToken.slice(-2)}'`);
    ...
    return getAllEntries(client).then(entries => {
        return getAllAssets(client).then(assets => ({entries, assets}));
    }).then(({entries, assets}) => {
        logger.debug('Serverless: got entries from Contentful');
...
function filterAndTransformProperties(entries, assets, options) {
    ...
    if (!options.allObjects) {
        entries = filterRootEntries(entries, _.property('fields.stackbit_model_type'));
    }
...
function transformEntries(entries, entriesById, assetsById, options) {
    ...
    return _.map(entries, entry => {
        ...
        return deepMap(entry, ({value, keyPath: fieldPath}) => {
