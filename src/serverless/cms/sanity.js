...
function buildCMS(projectId, ssgType, token, options) {
    logger.debug(`Serverless: fetching entries from Sanity, projectId: '${projectId}', token: '${token.slice(0, 2)}..${token.slice(-2)}', dataset: ${options.dataset}`);
    ...
    options = _.assign({
        includeMetadata: shouldIncludeMetadata(options),
        projectId: projectId
    }, options);
    ...
    return client.fetch(`*[${query}]`).then((entries) => {
        logger.debug('Serverless: got entries from Sanity');
        if (preview) {
